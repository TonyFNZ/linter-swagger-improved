...
function populateRangesForErrors( errors, text ) {
  ...
  try {
    ...
    // Unable to parse document, so cannot assign ranges
    return errors;
  }

  errors.forEach( ( error ) => {
    if ( error.path && error.path.length ) {
      error.range = findRangeRecursive( docRoot, error.path.slice( 0 ) ); // eslint-disable-line no-param-reassign
    } else {
      error.range = undefined; // eslint-disable-line no-param-reassign
    }
  } );
